...
ridge_pred_actual = np.expm1(ridge_pred)
ridge_pred_actual = np.clip(ridge_pred_actual, 0, None)

ridge_rmse = np.sqrt(mean_squared_error(y_test_actual, ridge_pred_actual))
ridge_mae = mean_absolute_error(y_test_actual, ridge_pred_actual)
...
ridge_cv = cross_val_score(ridge_model, X_train_scaled,
                            y_train, cv=5, scoring='r2')
print(f"\nCross Validation R² (5-fold): {ridge_cv.mean():.4f} ± {ridge_cv.std():.4f}")

# Ridge Actual vs Predicted
plt.figure(figsize=(8, 6))
plt.scatter(y_test_actual, ridge_pred_actual,
...
plt.legend()
plt.tight_layout()
plt.savefig('Img/ridge_actual_vs_predicted.png', dpi=150)
plt.close()
print("Ridge chart saved!")
...
print(f"{'Ridge Regression':<20} {ridge_r2:>8.4f} ${ridge_rmse:>14,.2f} ${ridge_mae:>14,.2f}")
print(f"{'KNN':<20} {knn_r2:>8.4f} ${knn_rmse:>14,.2f} ${knn_mae:>14,.2f}")
print("=" * 62)
print("\n✅ Best Model: Random Forest (lowest RMSE)")

# Final Comparison Chart
models = ['Random Forest', 'Ridge Regression', 'KNN']
r2_scores = [rf_r2, ridge_r2, knn_r2]
rmse_scores = [rf_rmse, ridge_rmse, knn_rmse]
mae_scores = [rf_mae, ridge_mae, knn_mae]
